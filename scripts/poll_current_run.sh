...
# ── GUARDRAIL: worker-commit verification ──────────────────────────────────
# Python imports are cached per-process. A running Celery worker holds its
# source modules in RAM — 'git pull' does NOT reload them. If the worker
# running the job is on a different commit from the manifest, this run's
# events file was generated by the WRONG code and its metrics are invalid.
#
...
PYEOF
    ...
    if "$PY" "$REPO/$EVAL_SCRIPT" "$EVENTS_FILE" \
      ...
      "$PY" - "$EVAL_JSON" "$RESULT_FILE" "$MANIFEST" "$EVAL_LOG" "$WORKER_COMMIT_FILE" "$MISMATCH_FLAG" <<'PYEOF'
import json, os, sys, datetime
eval_json, dest, manifest, log_path, worker_file, mismatch = sys.argv[1:7]
...
if os.path.exists(worker_file):
    try:
        worker_sha = open(worker_file).read().splitlines()[0].strip()
    except Exception:
        pass
payload = {
    ...
    "worker_commit_matches_manifest": mismatch == "0",
    "status": "success",
