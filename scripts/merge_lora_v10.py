"""Streaming LoRA merge for v10 LoRA into the v8 merged-bf16-c300 base.
...
Usage:
    python merge_lora_v10.py [CHECKPOINT_TAG]   # e.g. c300 (default)
"""
import json
import shutil
import sys
...
V10_ROOT = Path("/mnt/transit/soccer-finetune/checkpoints/v10-32b")
BASE = Path("/mnt/transit/soccer-finetune/checkpoints/v8-32b/merged-bf16-c300")
OUT = V10_ROOT / f"merged-bf16-{CHECKPOINT_TAG}"
OUT.mkdir(parents=True, exist_ok=True)

run_dirs = sorted(V10_ROOT.glob("v0-*"))
if not run_dirs:
    sys.exit(f"no v10 training run dirs under {V10_ROOT}")
RUN = run_dirs[-1]
