"""Inject v9b ball-trajectory + acceleration profile annotations into v6 swift dataset prompts.

v11 differs from v10 by adding an acceleration profile line to the
[ball-track] block. The hypothesis (per the v10 disambiguation eval,
Runs 85/89-92/93-96): coordinate-based trajectory features plateaued at
1/16 new-venue goal recall across c50/c150/c200. Acceleration encodes the
SHAPE of the trajectory — sharp deceleration distinguishes goal/save from
...
Writes: /mnt/transit/soccer-finetune/lora_dataset_v11_swift/{train,val,smoke,val_small}.jsonl
...
DST_DIR = Path("/mnt/transit/soccer-finetune/lora_dataset_v11_swift")
...
def _process_split(model, split: str) -> tuple[int, int]:
    ...
    with open(src) as f_in, open(dst, "w") as f_out:
        for line in f_in:
            ...
            summary = format_track(traj, include_acceleration=True)
            ex["query"] = _inject(ex["query"], summary, prompt_prefix(include_acceleration=True))
